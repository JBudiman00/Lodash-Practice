...
class ArtistCount{
    artistName: string;
    count: number;
    constructor(aP: string, num: number)
    {
        this.artistName = aP;
        this.count = num;
    }
...
function novemberArtist(){
    let arrArtist: ArtistCount[] = [];

    let nameFound = false;
    let ct = 0;

    _.times(abbeyRoadRecords.length, () =>{
        nameFound = false;
        arrArtist.forEach(a => {
            if(a.artistName === _.get(abbeyRoadRecords[ct], 'artist')){
                if (_.get(abbeyRoadRecords[ct], 'month') == 11){
                    a.count++;   
                }
                nameFound = true;
            }
        ...
        {
            if (_.get(abbeyRoadRecords[ct], 'month') == 11){
                arrArtist.push(new ArtistCount(_.get(abbeyRoadRecords[ct], 'artist'), 1))
            }
            else
                arrArtist.push(new ArtistCount(_.get(abbeyRoadRecords[ct], 'artist'), 0))
        }
        ct++;
    });
...
function getHighest(array, highTotal) { //Get array to find highest, definition of holder to get highest total object
    array.forEach(b => {
        if (b.count == highTotal[0].count){
            highTotal.push(b);
        }
...
function lastBeatlesSong() {
    return _.findLast(abbeyRoadRecords, e => {
        return _.get(e, 'artist') === 'The Beatles'
    });
}
...
class ArtistYear{
    ...
    artistYr: number;
    artistMonth: number;
    constructor(aN:string, aY: number, aM:number){
        this.artistName = aN;
        this.artistYr = aY;
        this.artistMonth = aM;
    }
...
function lastSixties() {
    let ct = 0;
    let artist60s: ArtistYear[] = [new ArtistYear("dummy", 1959, 0)];
    _.times(abbeyRoadRecords.length, () =>{
        if (_.get(abbeyRoadRecords[ct], 'year') > 1959 && _.get(abbeyRoadRecords[ct], 'year') < 1970 )  //First condition: Is it in 1960s?
            if (_.get(abbeyRoadRecords[ct], 'year') >= artist60s[0].artistYr)  //Second conditions: Is current artist year higher than/equal to previous highest artist's 1960s year?
                if (_.get(abbeyRoadRecords[ct], 'month') == artist60s[0].artistMonth)
                    artist60s.push(new ArtistYear(_.get(abbeyRoadRecords[ct], 'artist'), _.get(abbeyRoadRecords[ct], 'year'), _.get(abbeyRoadRecords[ct], 'month')));
                else if (_.get(abbeyRoadRecords[ct], 'month') > artist60s[0].artistMonth){
                    artist60s = [];
                    artist60s.push(new ArtistYear(_.get(abbeyRoadRecords[ct], 'artist'), _.get(abbeyRoadRecords[ct], 'year'), _.get(abbeyRoadRecords[ct], 'month')));
                }
        ct++;
